...
class TransformerHparams(object):
  ...
  def get_train_flops(self, batch_size, train_steps, discriminator=False):
    ...
    forward = batch_size * train_steps * (
...
for num_gpus in [4, 8, 16, 32, 64, 128, 256, 512]:
  bs = num_gpus * 8
  num_tokens = bs * 2048 * 10000
  flops = TransformerHparams( 512, 24, s=2048, v=50264, heads=32, output_frac=1.0, sparse_embed_lookup=False, decoder=False).get_train_flops(bs, 10000)
  MODEL_FLOPS[num_tokens] = flops
...
#     ("gpt", TransformerHparams( 512, 24, s=2048, v=50264, heads=32, output_frac=1.0, sparse_embed_lookup=False,
#       decoder=False).get_train_flops(
#         bs, 10000)),
